...
public class UserResource {
	...
	{
		String status=userService.lentBook(customerBook);
		logger.info(status);
		return status;
	}
	...
	{
		String status=userService.returnBook(customerBook);
		logger.info(status);
		return status;
	}
	...
	@Consumes("application/json")
	@Produces("text/html")
	public String changePassword(UserVO userDetails)throws SQLException
	{
		logger.info(userDetails.getUserId()+" "+userDetails.getPassword());
		String status=userService.changePassword(userDetails);
		logger.info(status);
		return status;
	}
	...
	{
		logger.info("called me in view books resource, "+customerId+" find me");
		PaginationResult pagination= userService.viewLentBooks(customerId);
		logger.info("process done in pagination"+pagination.getCount()+" "+pagination.getUnique_id());
		return pagination;
	}
	...
	public String addCustomer(UserVO user_details) throws SQLException{
		String status=userService.getRegister(user_details);
		logger.info(status);
		return status;
	...
	public String getPagination(@QueryParam("unique_id") int unique_id,@QueryParam("offset") int start,@QueryParam("end") int end) throws SQLException, IOException, TemplateException
	{
			...
			return userService.paginationCustomerResults(new PaginationResult(unique_id,start,end));
